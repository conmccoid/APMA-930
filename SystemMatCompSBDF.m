function [PsiOm] = SystemMatCompSBDF(numUn, nP, nO, M, N, alpha, dr, dth, a)
% does sparse storage
...
%  Interior points - 5 point stencil
        for jrow = 2: N-1
            ...
            PsiOm(k+1) = -a*(r_i - 0.5*dr)/(r_i*dr^2);
            PsiOm(k+2) = -a*(r_i + 0.5*dr)/(r_i*dr^2);
            PsiOm(k+3) = -a/(r_i*dth)^2;
            PsiOm(k+4) = -a/(r_i*dth)^2;
            PsiOm(k) = 1 + a*(2/dr^2 + 2/(r_i*dth)^2);
            k = k + 5;
        end
%
