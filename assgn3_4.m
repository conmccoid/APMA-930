...
lx = 1;       % width of box
ly = 1;       % height of box
nx = a*lx;      % number of x-gridpoints
...
for k = 1:nt
   ...
   P = reshape(p,nx,ny);
   dP = diff(P)/hx;
   index = 1:nx; index = index(abs(x - 0.5)<=hx);
   if length(index)==2
       dP = dP(index(1),:)+dP(index(2),:); dP = dP/2;
   else
       dP = dP(index,:);
   end
   minimum = min(dP);
%    figure(2)
%    hold on
%    plot(log(nx),log(abs(min(dP)-minimum)),'*','Markersize',10)
%    hold off
   U = U-dt*diff(P)/hx;
   V = V-dt*diff(P')'/hy;
...
%    % testing stability
%    t = t+dt;
%    testU = norm(U - U_old)/norm(U);
%    testV = norm(V - V_old)/norm(V);
%    if testU<=tol && testV<=tol && results~=1
%        disp(num2str(t))
%        results=1;
%    end
%    V_old = V;
%    U_old = U;
   ...
   if k==1||floor(nsteps*k/nt)>floor(nsteps*(k-1)/nt)
       figure(1)
      % stream function
      rhs = reshape(diff(U')'/hy-diff(V)/hx,[],1); % vorticity?
      q(perq) = Rq\(Rqt\rhs(perq));                % stream function
      Q = zeros(nx+1,ny+1);
      Q(2:end-1,2:end-1) = reshape(rhs,nx-1,ny-1); % visualize vorticity or stream function (first input)
